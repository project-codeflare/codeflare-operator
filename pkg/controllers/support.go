...
func controllerSpecificHash(s string) string {
	return fmt.Sprintf("%x", sha256.Sum256([]byte(hashConstant+s)))[:hashLength]
}

func getUniqueName(s string) string {
	return s + "-" + controllerSpecificHash(s)
}
